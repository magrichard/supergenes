...
if (!exists("study_dmeth_tcga_luad")) {
  ...
  s_luad = epimedtools::create_study("~/projects/supergenes/data/tcga_studies/study_TCGA-LUAD_meth.rds")
...
if (!exists("study_dmeth_tcga_lusc")) {
  ...
  s_lusc = epimedtools::create_study("~/projects/supergenes/data/tcga_studies/tudy_TCGA-LUSC_meth.rds")
...
study_meth_all_human_tissues_filename = "~/projects/supergenes/data/all_human_tissues/study_meth_all_human_tissues.rds"
...
study_meth_nt_tcga_filename = "~/projects/supergenes/data/tcga_studies/study_nt_tcga_meth.rds"
...
if (!exists("epic")) {
  ...
  epic_orig = readRDS("~/projects/supergenes/data/all_human_tissues/EPIC.hg38.manifest.gencode.v22.rds")
...
The purpose is to observe epigenetic profile of *features* provided by EpiMed database.

Provided *features* are: *epimed_tsg2_restricted_1_testis*
...
if (!exists("features")) {
  print("Loading featrures from EpiMed database")
  prefix = "epimed_tsg2_restricted_1_testis_adult"
  # get entrez
  if (!file.exists("features_ts.rds")) {
    print("Loading features from epimeddb...")
    url = "http://epimed.univ-grenoble-alpes.fr/database/query/genes?annotations=epimed_tsg2_restricted_1_testis_adult&taxid=9606"
    entrez = read.csv2(url, header=TRUE, sep=";", stringsAsFactors=FALSE)$entrez  
    # get position
...
```{r}
genes = readRDS("~/projects/supergenes/data/bed_grch38_epimeddb.rds")

# locate
chrs = unique(features[,1])[order(as.numeric(substr(unique(features[,1]),4,100)))]
...
sum(rownames(features) %in% rownames(genes))
stoda::locate(gs=rownames(features), chrtss=chrtss)
axis(2,labels=rev(chrs), at=1:length(chrs), las=2)


# # grps = apply(cutree(hc_row, k = c(3,5)), 1, paste, collapse="_")
# layout(matrix(1:6, 2), respect=TRUE)
# for (i in sort(unique(grps))) {
#   idx = names(grps)[grps == i]
#   # df = features[idx_meth_genes,]
#   # WriteXLS::WriteXLS(df, paste("meth_genes.xlsx", sep=""), row.names=TRUE)
#   # saveRDS(df, "meth_genes.rds")
# }
```
...
```{r echo=FALSE, results="verbatim"}
ts_features  = features[rownames(meth_by_tissues_by_feature),]
genes = readRDS("~/projects/supergenes/data/bed_grch38_epimeddb.rds")
set.seed(1)
rnd_features  = genes[sample(rownames(genes[genes$tss_cpg_status == "cpg_rich",]), 500),]
features_1 = ts_features [,1:6]  
...
source(knitr::purl("~/projects/all_human_tissues/results/meth/00_deeptools_call.Rmd"))
```
