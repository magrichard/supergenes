...
if (!exists("study_dmeth_tcga_lusc")) {
  ...
  s_lusc = epimedtools::create_study("~/projects/supergenes/data/tcga_studies/study_TCGA-LUSC_meth.rds")
...
reduce_rows = function(tmp_meth_data, map,  indicator_func2=mean, ...) {
  ...
  meth_by_tissues_by_feature = epimedtools::monitored_apply(mod=10, t(t(names(map))), 1, function(f) {
    ...
    # f = features[1,]
    # f = features["HACD4",]
    probe_idx = intersect(rownames(tmp_meth_data), rownames(map[[f]]$sub_epic))
  ...
  names(meth_by_tissues_by_feature) = names(map)
...
source(knitr::purl("~/projects/supergenes/data/00_deeptools_call.Rmd"))
```
