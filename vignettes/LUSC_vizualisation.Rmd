...
Our binmap is now a list of one level depth, of length bins times features (`length(binmap)`)
...
We transformed reduce a database of `r nrow(meth_lusc$data)` probes for `r ncol(meth_lusc$data)`  patients into a database of `r nrow(means_per_bins_per_genes_per_patient)` rows.
