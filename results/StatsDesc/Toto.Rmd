...
source("~/projects/supergenes/funMilan.R")
...
```{r}
features = get_features(penda_superconserved,index_pathology = 2,trscr_lusc, up_str = 2500, dwn_str = 2500, nb_probe_min = 1)
```
...
```{r}
plot_selected_gene("TG",expr_data = trscr_lusc$data, meth_data = meth_lusc$data, probes_index = probes_index)
```



```{r}
plot_selected_probes(selected_gene)
```
