...
plot_selected_regions<- function(selected_gene = "ALDH3B1",
  ...
  cols<-sapply(rownames(sub_epic), function(probe){
    ...
    if(is.element(probe,rownames(epic_450k))){
      col ="darkcyan"
    }
    
    if(is.element(probe,rownames(epic_27k))){
      
      col = "chartreuse4"
    }
    if(is.element(probe,rownames(epic_27k)) & is.element(probe,rownames(epic_450k))){
      
      col = "red"
    }
    if(!is.element(probe,rownames(epic_850k)) & is.element(probe,rownames(epic_450k))){
  ...
  legend("topright", c("450k-850k","27k","27k-450k","450k"),inset=c(-0.05,0), xpd=TRUE, pch = 19, col = c("darkcyan","chartreuse4","red","black"), bty="n",title ="Chip")
