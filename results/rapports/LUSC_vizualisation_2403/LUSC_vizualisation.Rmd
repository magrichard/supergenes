...
features = get_features(penda_superup_deregulated, trscr_lusc, up_str = 7500, dwn_str = 7500, nb_probe_min = 1)
```
...
```{r}
get_features_bins <- function(targeted_genes, study = trscr_lusc, up_str = 7500, dwn_str = 7500, nb_probe_min = 1, ...) { #### In my uses-case, 1 for LUAD & 2 for LUSC
  print("Creating a list of features...")
  features <- study$platform[intersect(rownames(study$platform), targeted_genes), ]
...
```{r}
subset_vals_per_bins <- function(data = meth_lusc$data, values_per_patient = means_per_bins_per_genes_per_patient, binlist = c("bin1","bin2","bin3","bin4","bin5","bin6"), fun = mean, names = feat_indexed_probes, ...){
 ...
 ret <- list()
  ...
  rownames(vals_per_genes) <- names(feat_indexed_probes) 
